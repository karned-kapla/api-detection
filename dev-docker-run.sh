...
# This script creates a development Docker container for api-object-detection
...
docker stop karned-api-object-detection || true
docker rm karned-api-object-detection || true

# Run the container with local directory mounted
docker run -d \
  --name karned-api-object-detection \
  ...
  -e API_NAME=api-object-detection \
  -e API_TAG_NAME=object-detections \
  killiankopp/api-object-detection:1.0.0 \
...
echo "To view logs: docker logs karned-api-object-detection"
echo "To stop: docker stop karned-api-object-detection"
